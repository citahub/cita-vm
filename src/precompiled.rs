...
use numext_fixed_hash::{H160 as Address, H256, H512};
...
pub fn get(address: Address) -> Box<PrecompiledContract> {
    let v = evm::common::address_to_u256(address);
    match evm::common::u256_to_u64(v) {
...
pub fn contains(address: &Address) -> bool {
    let i = evm::common::address_to_u256((*address).clone());
    i <= U256::from(8 as u32) && !i.is_zero()
}
...
fn is_signature_valid(r: &H256, s: &H256, v: u8) -> bool {
    v <= 1
        && *r < H256::from_hex_str("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141").unwrap()
        && *r >= H256::from(U256::from(1 as u32))
        && *s < H256::from_hex_str("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141").unwrap()
        && *s >= H256::from(U256::from(1 as u32))
}
...
fn recover(input: &[u8], hash: &[u8], bit: u8) -> Result<H512, secp256k1::Error> {
    ...
    Ok(H512::from_slice(&pub_key_ser[1..65]).unwrap_or_default())
}
...
impl PrecompiledContract for EcRecover {
    ...
    fn run(&self, i: &[u8]) -> Result<Vec<u8>, err::Error> {
        ...
        let hash = H256::from_slice(&input[0..32]).unwrap();
        let v = H256::from_slice(&input[32..64]).unwrap();
        let r = H256::from_slice(&input[64..96]).unwrap();
        let s = H256::from_slice(&input[96..128]).unwrap();

        let bit = match v.byte(31).unwrap() {
            27 | 28 if v.0[..31] == [0; 31] => v.byte(31).unwrap() - 27,
            _ => {
                return Ok(vec![]);
            }
        };
