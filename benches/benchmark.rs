...
fn bench_raw_new_contract(bench: &mut Bencher) {
    bench.iter(|| {
        let db = Arc::new(cita_vm::state::MemoryDB::new(false));
        let mut state = cita_vm::state::State::new(db).unwrap();
        ...
        for _ in 0..1000 {
            ...
            let code = random_bytes;
            state.new_contract(&address, U256::from(10), U256::from(1), code.to_vec());
        }
        state.commit().unwrap();
    });
}
...
fn bench_new_contract(bench: &mut Bencher) {
    ...
    bench.iter(|| {
        ...
        println!("Executing tx: {:?}", SystemTime::now().duration_since(tic));

        let tic = SystemTime::now();
        state_data_provider.borrow_mut().commit().unwrap();
        println!("Commiting tx: {:?}", SystemTime::now().duration_since(tic));
    })
}

benchmark_group!(benches, bench_count, bench_raw_new_contract, bench_new_contract);
